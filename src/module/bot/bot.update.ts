...
import { BotService, games } from "./bot.service";
...
export class BotUpdate {
    ...
    async onStart(ctx: Context) {
        ...
        if (!user) {
            user = await this.userService.createUser({ uid: ctx.from.id, name: ctx.from.first_name })
        }
    ...
    async getCallbacks(@Ctx() ctx: Context) {
        // const { command, payload } = JSON.parse(ctx.update.callback_query.data) || {}
    ...
    async userProfile(ctx: Context) {
        await ctx.reply('this is your profile, you can change him', profileButtons())
    }
}
